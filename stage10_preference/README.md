...
## 10.3 chat-template SFT:同格式对比补完(stage11.1 的数据接 10 的工艺)

**动机**:11-mm 在 raw 格式破了 0(≈4/20),但 9.5 的对比对象(官方 64M)
是在 **chat 格式**下测的——格式不同不能比。10.3 给 11-mm 补 chat-template
SFT(官方 sft 语料渲染 → 我们的 BPE 编码 → assistant 段掩码),把
"同格式对打"补上。数据:官方 `sft_t2t_mini.jsonl`(905,718 行 conversations),
取单轮对 40k 行,用官方 tokenizer 的 apply_chat_template 渲染成三段式
(user_side / answer / tail),tail 含 `<|im_end|>` 让模型学会收尾。

### 一次数据工艺消融(意外发现,很有教学价值)

第一版直接渲染训练:**2/20(真 ≈1)**——检查发现抽到的 40k 行**全部带
`<think>` 块**,而官方预处理(post_processing_chat)会把空 think 以 80%
概率剥掉,我们没剥 → 模型学到"先空想、再绕圈子列条"的坏风格,20 题
直答全毁。剥掉 think 块重训(800 步):

| 模型 | chat 20 题 | 真实答对 |
|---|---|---|
| 10-chat(带 think 噪音) | 2/20 | ≈1(珠峰真阳;秦始皇假阳) |
| **10-chat-nt(去噪)** | **4/20**(高 3 低 1) | **≈3**(故宫→北京市 ✓ 珠峰→完整含海拔 ✓ 圆周率→3.14159 ✓;秦始皇复读假阳;8848 漏判——它答珠峰时带了"8,848.8 米") |

**数据工艺值 2 分**:同样的模型同样的步数,只剥掉 think 噪音,真阳 1 → 3。

### 同格式对比(10.3 的核心答案)

| | 我们 10-chat-nt | 官方 minimind-3 64M |
|---|---|---|
| 格式 | chat(同一套模板文本) | chat(母语) |
| 真实答对 | **≈3/20**(另有 8848 漏判) | ≈5/20(9.5 行级审计) |
| 预训练数据 | mini 1.2GB × 0.27 epoch | 主线 10GB 完整训练 |
| SFT 剂量 | 40k 行 × 800 步 | 全套 chat SFT(行数与步数远大于我们) |

**结论:同格式、同 100M 级,我们用 1/8 预训练数据 + 一小撮 SFT 达到官方
~60% 的真实答对率**——11.1 的"数据剂量直接兑换能力"在此再次量化;
剩余差距归于剂量(10GB/全套 SFT/多轮数据),不是未知机制。

### 额外观察

- 收尾机制工作:模型学会发 `<|im_end|>`(评估按它截断,无需我们的 EOS)
- 复读尾部仍是贪心生成的通病(3.14159×N),解码侧可治
- raw 知识是否被 chat SFT 覆盖未复测(诚实开口;11-mm 的 raw ≈4/20 是
  上一基座的状态,chat_nt 是它 SFT 后的新权重)
...
看齐主线的剩余差距都是"剂量"性质:预训练 10GB 主线档 × 长步数(stage11.2 候选)、
全套 SFT 数据与多轮模板、DPO/GRPO 接真实偏好数据。可选池(MoE/蒸馏/HF 生态化)
随时可开。ROADMAP 主线的机制学习部分已全部闭环。
