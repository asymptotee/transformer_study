...
## 9.5 桥接实验:官方 minimind-3 跑同一套 20 题

**目的**(9.0 就备好的外部检验):我们的 94_full 答 20 常识题真实 ≈0/20。
官方 minimind-3 dense(63.9M)在同一套题上表现如何?分离"架构(9.4 后
同族)/ 规模(64M vs 117.8M)/ 数据量(10GB 官方语料 vs 150MB)";
顺手检验 stage8 的三层归因。

**格式公平性设计**(stage8/9.0 的模板教训):raw 格式(问：…\n答：)是我们的
训练分布;chat 格式(官方 qwen3 系 chat template)是官方模型的训练分布。
两个格式都跑,格式错配导致的失败不算"不会"。

| 模型 | 参数 | 数据 | raw 格式 | chat 格式 |
|---|---|---|---|---|
| 94_full(我们,预训练 only) | 117.8M | 150MB wiki(44M token) | ≈0/20(真实) | 无此格式能力 |
| **官方 minimind-3**(SFT 后) | 63.9M | **10GB**(pretrain_t2t,主线) | **0/20**(复读冒号,格式错配) | **6/20**(高频 4 + 低频 2) |

### 行级观察(chat 格式)

- **真阳 ≈5/20**(1 个假阳):故宫→"中国北京市东城区,明清两代的皇家宫殿"、
  珠峰、熊猫→四川、一战→1914(都答得完整流利);秦始皇给对朝代但年份胡写
  ("公元前256年-公元前2100年");水的化学式先答"CO2"再胡扯出 H2O(假阳)
- **它的错法也是小模型典型**:中国的首都→"四川省"、第一大城市→北京、
  长城位于→"西部地区"——高频共现网络里"路径碰对"与"张冠李戴"并存
- raw 格式下输出直接崩成"：：：…"复读——**格式=分布,双向成立**:我们答
  不出它的 chat 分布,它答不出我们的 raw 分布

### 结论:stage8 归因需要修正一条

1. **"检索之坎"的主闸门不是参数规模**(stage8 归因二需要修正):官方 64M——
   我们一半的参数——在 chat 格式下能定点取出 5 题(含 2 道低频)。0 vs 5
   的主因是:**① 数据量**(10GB vs 150MB,约两个数量级;它"读过"故宫位于…
   北京的句式与海量问答分布)+ **② SFT 行为链**(提问→取知识→作答)。
2. **stage8 归因一(测试题不在语料分布)升级为通用规律**:任何模型都只会在
   自己训练分布覆盖的"提问方式"下作答;模板错配 = 能力归零(双向证据)。
3. **小模型检索质量的真实天花板仍在**:64M 答 5/20 且错误有规律(首都↔
   四川)——知识是"高频共现网络",不是事实数据库;与 stage7/8 的画像一致。
4. **对 stage10 的指向**:数据是第一瓶颈。想在自家基座上看到 >0 的常识分,
   扩语料(10GB 级)比加参数更有效;或者走 RAG 绕开"记忆"。

局限(诚实记录):官方仓库未提供纯预训练 base pth(下载列表为 full_sft/
agent/dllm),"数据量"与"SFT"两个变量未完全分离——raw 0/20 里含 SFT
模板锁定的贡献;严谨拆分需要官方 base,留作可选项。

### 本里程碑文件

| 文件 | 内容 |
|---|---|
| `eval_minimind.py` | 桥接评估(HF/pth 双加载,raw/chat 双格式) |
| `results/minimind3-hf.json/log` | 官方权重评估明细 |
| Spark `~/llm_study/mm_weights/` | 官方权重与仓库(不入库) |

## 下一步

阶段 9(架构看齐)收官:现代基座 `ckpt_94_full.pt`(val 3.776,比 stage7
−0.12)+ 完整零件履历(表 A/B)。进入阶段 10:偏好学习(DPO 必做,
GRPO 推荐)——现代基座上造偏好数据、自己推 DPO loss、与 minimind
`train_dpo.py` 的 loss 数值对齐。见 ROADMAP。
